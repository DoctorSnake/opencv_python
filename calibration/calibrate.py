...
class CameraCalibration:
	...
	def findMarkers(self,gray,objpoints,imgpoints):
		...
		if self.marker_checkerboard == True:
			...
			ret, corners = cv2.findCirclesGridDefault(gray, self.marker_size,None,cv2.CALIB_CB_ASYMMETRIC_GRID)
...
def handleArgs():
	...
	parser.add_argument('-s', '--target_size', type=int, nargs=2, help='size of pattern, for example, (6,7)', default=(7,6))
	parser.add_argument('-p', '--path', help='location of images to use', required=True)
	parser.add_argument('-d', '--display', help='display images', default=False)
